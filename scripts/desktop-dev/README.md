...
也可以在 `frontend\taichi-flow` 中运行 `npm run desktop:dev`、`npm run desktop:preview` 或 `npm run desktop:smoke`。高级参数（端口、超时、DevTools、Smoke 证据路径）直接传给 `Start-DesktopDev.ps1`。
...
端口只有在健康响应、API 契约、checkout 指纹和监听进程来源都一致时才会复用。无关占用不会被结束，启动器会选择后续空闲回环端口。会话、PID 身份、所有权和日志位于仓库 `.runtime\desktop-dev\`；陈旧记录不会被当作杀进程依据。
