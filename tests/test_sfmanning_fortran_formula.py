...
def test_sfmanning_decomposition_exposes_fortran_formula_terms():
    ...
    params = record["sfmanning_parameters"]
    assert components["sfmanning_current"]["sum"] > 0.0
    assert components["manning_field"]["max"] > 0.0
    assert components["manningbar_current"]["max"] > 0.0
    assert components["absubar_current"]["max"] > 0.0
    assert params["depth_exponent_fortran_literal"] == 1.333


def test_sfmanning_variants_include_disabled_and_fortran_velocity_candidates():
    solver = build_two_cell_erosion_solver(erodible_thickness=10.0)
    cfg = solver.config
    ...
    variants = record["sfmanning_variants"]
    assert set(variants) == {
    ...
    assert variants["A_active_current_tau"]["count_all_erosion_gates_true"] > 0
    assert variants["B_sfmanning_disabled"]["predicted_erosion_increment_sum"] >= 0.0
